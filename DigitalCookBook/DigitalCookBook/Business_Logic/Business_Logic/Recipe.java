...
public class Recipe implements Serializable {
	...
	public void reviseServings( int servings) {
		int buffer = this.getServings();

		this.setServings(servings);

		int oldPT = this.getPreparationTime();

		int oldCT = this.getCookingTime();

		this.setPreparationTime(oldPT * servings / buffer);

		this.setCookingTime(oldCT * servings / buffer);

		for (int i = 0; i <= this.getIngredient().size() - 1; i++) {

			double oldQuantity = this.getIngredient().get(i).getQuantity();
			this.getIngredient().get(i).setQuantity(oldQuantity * servings / buffer);
